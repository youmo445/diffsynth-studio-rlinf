...
class Encoder3d(nn.Module):
    ...
    def __init__(self,
        ...
        for i, (in_dim, out_dim) in enumerate(zip(dims[:-1], dims[1:])):
            print(f'Downsample block {i}: in_dim={in_dim}, out_dim={out_dim}')
            # 96 - 96
            # 96 - 192
            # 192 - 384
            # 384 - 384
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        if feat_cache is not None:
            ...
            idx = feat_idx[0] # 0
            cache_x = x[:, :, -CACHE_T:, :, :].clone()
            #print(f'cache_x.shape before cat: {cache_x.shape}')
            # [1, 3, 1, 256, 256]
            # [1, 3, 2, 256, 256]
            # feat_cache[0]: [1, 3, 2, 256, 256]
            if cache_x.shape[2] < 2 and feat_cache[idx] is not None:
                # cache last frame of last two chunk
                cache_x = torch.cat([
                    feat_cache[idx][:, :, -1, :, :].unsqueeze(2).to(
                        cache_x.device), cache_x
                                    ...
                                    dim=2)
                print(f'cache_x.shape after cat: {cache_x.shape}')
            ...
            #print(f'no feat_cache')
            x = self.conv1(x)

        ## downsamples
        for layer in self.downsamples:
            if feat_cache is not None:
                #print(f'feat_cache in downsamples')
                x = layer(x, feat_cache, feat_idx)
                #print(f'layer name: {layer.__class__.__name__}')
                #print(f'After downsample shape: {x.shape}')
            else:
                x = layer(x)
        # print(f'After downsamples shape: {x.shape}')
        ## middle
        for layer in self.middle:
            # 3层， ResidualBlock, AttentionBlock, ResidualBlock, 一直是[1, 384, 1, 32, 32]
            if check_is_instance(layer, ResidualBlock) and feat_cache is not None:
                x = layer(x, feat_cache, feat_idx)
            else:
                x = layer(x)

        # print(f'After middle shape: {x.shape}')
        ## head
        for layer in self.head:
            if check_is_instance(layer, CausalConv3d) and feat_cache is not None:
                # print(f'feat_cache in head')
                idx = feat_idx[0]
                cache_x = x[:, :, -CACHE_T:, :, :].clone()
                if cache_x.shape[2] < 2 and feat_cache[idx] is not None:
                    # cache last frame of last two chunk
                ...
                # 2层，一直是[1, 384, 1, 32, 32] ->
                # print(f'no feat_cache in head')
                x = layer(x)
...
class VideoVAE_(nn.Module):
    ...
    def __init__(self,
        ...
        # Encoder3d initialized: dim=96, z_dim * 2 =32, dim_mult=[1, 2, 4, 4], num_res_blocks=2, attn_scales=[], temperal_downsample=[False, True, True], dropout=0.0
        # import pdb; pdb.set_trace()
    ...
    def encode(self, x, scale):
        ...
        for i in range(iter_):
            ...
            if i == 0:
                # print(f'Encoding chunk {i}, frames {0} to {1}')
                out = self.encoder(x[:, :, :1, :, :],
                                   feat_cache=self._enc_feat_map,
                                   feat_idx=self._enc_conv_idx)
                #print(f'Encoded shape: {out.shape}')
                # [1, 32, 1, 32, 32]
            else:
                #print(f'Encoding chunk {i}, frames {1 + 4 * (i - 1)} to {1 + 4 * i}')
    ...
    def encode_all(self, x, scale):
        ...
        iter_ = t // 4
        out_list = []
        #print(f'len of feat_cache: {len(self._enc_feat_map)}')
        for i in range(iter_):
            start = 4 * i
            end = 4 * (i + 1)
            x_block = x[:, :, start:end, :, :]   # 每段固定用4帧
            out_ = self.encoder(x_block, feat_cache=self._enc_feat_map, feat_idx=[0])
            out_list.append(out_)
            #print(f'Encoded shape block {i}: {out_.shape}')
        ...
        for i in range(iter_):
            ...
            if i == 0:
                ...
                out_ = self.decoder(x[:, :, i:i + 1, :, :],
                                    feat_cache=self._feat_map,
                                    feat_idx=self._conv_idx)
                #print(f'Decoded shape: {out_.shape}')
...
class WanVideoVAE(nn.Module):
    ...
    def encodeback(self, videos, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        ...
        for video in videos:
            ...
            #print(f'tiled: {tiled}')
            if tiled:
                raise TypeError(f"Should not use tiled")
                #print(f'tile_size: {tile_size}, tile_stride: {tile_stride}')
    ...
    def decodebackup(self, hidden_states, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
