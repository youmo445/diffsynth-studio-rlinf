...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        if self.has_image_input:
            img = y[:, :257]
            # print(f'CrossAttention img shape: {img.shape}')
            ctx = y[:, 257:]
            if ctx.shape[1]!=0:
                # print(f'CrossAttention ctx shape: {ctx.shape}')
                pass
        else:
            ctx = y
        q = self.norm_q(self.q(x))
        ...
        if ctx.shape[1]!=0:
            ...
            # print(f'CrossAttention k,v shape: {k.shape}, {v.shape}')
            x = self.attn(q, k, v)
        if self.has_image_input:
            k_img = self.norm_k_img(self.k_img(img))
            v_img = self.v_img(img)
            # print(f'q,k,v_img shapes: {q.shape}, {k_img.shape}, {v_img.shape}')
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs):
        # print(f'x shape in DiTBlock: {x.shape}')
        # [1, 5632, 1536]
        # print(f't_mod shape in DiTBlock: {t_mod.shape}')
        # [1, 6, 1536]
        ...
        # input('DITBlock forward debug pause...')
        return x
...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        ######## 下面是为了处理逐帧的控制信号 ########
        self.modulate_action = TransformerBlock(hidden_size=dim, mlp_ratio=4.0)
    ...
    def forward(self,
        ...
        if self.has_image_input:
            ...
            print(f'x shape after concat y: {x.shape}')
            clip_embdding = self.img_emb(clip_feature)
            print(f'clip_embdding shape: {clip_embdding.shape}')
            context = torch.cat([clip_embdding, context], dim=1)
            print(f'context shape after concat clip_embdding: {context.shape}')
        
        x, (f, h, w) = self.patchify(x)
        print(f'x shape after patchify: {x.shape}')
        freqs = torch.cat([
